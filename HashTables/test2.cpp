...
bool test2() {
  ...
  if (T1.getValue("Hello") != 17 || T1.getValue("abc")!=12 || T1.getValue("AAAA") != 39 || T1.getValue("A")!=76) {
    cout << "Values are not stored correctly 3." << endl;
    return false;
  }
  ...
  for (int i=0; i<num; i++) {
    if (i%2) {
      if (T3.getValue(9*i*i+11*i+13) != 2*i-1) {
	cout << "Integer values are not stored correctly after erasing and reinserting 1." << endl;
	return false;
      }
    } else {
      if (T3.getValue(9*i*i+11*i+13) != 3*i-1) {
	cout << "Integer values are not stored correctly after erasing and reinserting 2." << endl;
	return false;
      }
    }
  }
